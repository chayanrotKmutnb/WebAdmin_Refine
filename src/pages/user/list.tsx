    ...
    <TableContainer>
            ...
            <Tr key={user.id}>
              {columns.map((column) => {
                if (column.accessorKey === "profileImageUrl") {
                  const truncatedUrl = truncateString(
                    user[column.accessorKey],
                    25
                  );
                  return (
                    <Td key={column.accessorKey}>
                      <Link href={user[column.accessorKey]} isExternal>
                        {truncatedUrl}
                      </Link>
                    </Td>
                  );
                  ...
                  return (
                    <Td key={column.accessorKey}>{renderActions(user.id)}</Td>
                  );
                } else {
                  return (
                    <Td key={column.accessorKey}>
                      {user[column.accessorKey as keyof IUser]}
                    </Td>
                  );
                }
              })}
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>Are you sure do you want to delete this user?</ModalBody>
          <ModalFooter>
            <Button
